...
Kentico Software is an IT company based in Brno, developing an all-in-one solution for web content management, e-commerce and online marketing using the ASP.NET\footnote{TODO} architecture. The leading product is Kentico CMS (Content Management System) that has been on the market since 2004 with 11th version currently being developed. 

Over the course of almost 13 years, the solution grew to an enormous extend. To give an overview, the current version of Kentico CMS solution contains XY projects and a total of YZ source files. A lot of knowledge was accumulated with many internal utilities developed and guidelines created. Many features, such as globalization, were developed in-house first, only to be added as an integral part of the .NET Framework a few years later.
...
BugHunter is a simple console application developed at Kentico that searches for violations of internal rules and best practices in the the CMS solution. Given the file path to the solution folder, the BugHunter performs the checks and outputs the results to a file. Each issue found is reported with a short message plus additional information on its location in the code (source file and line number). It contains various checks not, only for C\#, but also a few for JavaScript (like no skipped tests allowed), ASPX\footnote{TODO} (preferred usage of Kentico controls to default ones) and XML files.

Following sections explain what were the downsides of the old BugHunter and why an advanced tool was needed.

\subsection{Need for Semantic Analysis}
The biggest disadvantage of the previous BugHunter is that the tool performs the checks purely by string comparison. It does not build any model of the analyzed code and therefore possesses no knowledge about the semantics. 

This causes an enormous number of false negatives. A typical example would be the tool trying to prevents the access to \texttt{Cookies} property of the \texttt{HttpRequest} object. The old BugHunter check looks something like this:
...
If the programmer uses the most common approach\footnote{Accessing the static property of the class that is handling the request.} of accessing the \texttt{Cookies} collection on the \texttt{Request} object everything is fine and the BugHunter correctly reports the problem. However, it has no means of detecting other possible accesses to the property. If the \texttt{Request} object is stored in a variable, or passed it to a function as an argument the accesses of the cookies collection will not be detected by BugHunter. 

There are other posibilities that can lead to false negatives when using just primitive string comparisons. For example, in C\# it would be perfectly okay to put an arbitrary number of whitespace characters between the object, the dot token, and the property being accessed. This can be solved by using a bit more complicated regular expression, but it still covers only a fraction of use cases. Things like using variable or even alias usings\footnote{https://msdn.microsoft.com/en-us/library/aa664765(v=vs.71).aspx} will still be missed by the tool. The only way to solve this is to introduce the semantics into the analysis which is a relatively easy task for new Roslyn anlyzers developed as part of this thesis.

\subsection{Ease of Use}
As mentioned in the Chapter~\ref{chap:static-code-analysis}, the key to success of a static code analysis tool is the ease of use. This means that running the tool should not be complicated and also that interpreting the results should be straightforward. If this is not the case, programmers will not use the tool or they will ignore the results it produces. 
...
In a discussion with developers at Kentico, they all admitted, that more often then not, they submit the changes to the version control system without running the BugHunter locally. 

The console application is run by the build server itself as part of the continuous integration. If the BugHunter detects any issues the developer who caused them is notified by an email and must fix them as part of the no-warning policy. This process prolongs the time it takes to finish the implementation and generates a demand for a more developer friendly solution. Since Roslyn analyzers are part of the Visual Studio IDE, programmers get an instant feedback and see the warnings before submitting the code without the need to run an external application.

\subsection{Suppressing the Results}
Another downside of the previous solution was the way certain portions of the code were excluded from the analysis. Since the BugHunter is a console application and the CMS solution is completely independent of it, the configuration had to be placed in BugHunter itself. This was deemed as not transparent, as it was not clear, why in one file certain code was okay whereas in the other it was reported as an issue without taking a look into the huge configuration file.
...
\bigskip\noindent
Following sections will present the new version of BugHunter analyzers implemented using new Roslyn compiler platform as implementation part of this thesis.
...
TODO Describe each project.. why 2 for analyzers

\subsection{Analyzer Categories}
The \texttt{Category} property of the \texttt{Microsoft.CodeAnalysis.DiagnosticDescriptor} class provides a way for semantically structuring the analyzers and giving an additional informatino on what type of problem the analyzers within the category are concerned with. The projects with analyzers use the categories also for folder structure with each folder contaning separate subfolders for analyzers and code fixes respectively.


The following sections briefly describe what categories were defined and what they represent. 

\subsubsection{Abstraction over Implementation}

\subsubsection{CMS API Guidelines}

\subsubsection{CMS API Replacements}

\subsubsection{CMS Base Classes}

\subsubsection{String and Culture}
