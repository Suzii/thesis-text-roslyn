...
Some of the optimizations were already briefly mentioned in the previous chapter: customized approach for analyzers in \textit{String and Culture} category instead of generic \texttt{MethodInvocationAnalyzer}; and the syntax heuristics utilized in \texttt{MethodInvocationAnalyzer}.

This section presents the iterative process of optimizing \texttt{SystemIOAnalyzer}, as well as its final version.
...
The logic, however, is more complicated, as the tree needs to be traversed in order to find the nodes to be inspected. On the other hand, if the document does not contain string \textit{".IO"} at all, there will be no diagnostic and its analysis can be skipped. The fastest version with syntax tree action took an over 9 seconds to perform the analysis. Even though it was faster than the V1, it was still not ideal.
...
None of the versions described above provided a significant performance improvement compared to V1. To understand the limitations of the analysis, two artificial analyzers were created, that registered an action on identifier name syntax nodes. The first one only registered an empty callback. The second one obtained the symbol of the inspected node from the semantic model and then did a dummy comparison of its name\footnote{This was just to make sure that the compiler actually did not skip the semantic model access as part of an optimization, if the obtained symbol had not been used at all.}. Neither analyzer raised any diagnostic in CMS solution. 

The results of these measurements were very interesting. While the empty callback took 0.54 seconds on average \footnote{Presented values are averages from 100 runs. The box plots can be seen in~Figure~\ref{fig:system-io-versions-boxplots}.}, the second one run for an average of 7.89 seconds. 
...
\begin{figure}
		\centering
			\includegraphics[scale=0.175]{img/system-io-versions-boxpolt-1}
		\caption{TODO change labels. Box plots of execution times of different versions of \texttt{SystemIOAnalyzer}.}
		\label{fig:system-io-versions-boxplots}
\end{figure}

The execution times of final analyzer (with non-cached version as well), V1 analyzer and artificial analyzers serving as baselines are depicted in a box plot diagram in Figure~\ref{fig:system-io-versions-boxplots}. It is clear that the cached version provides by far the best results and was chosen as the final implementation.
...
The version without the analyzers took an average of 5.09 minutes to build, whereas for the version with installed BugHunter analyzers it was 5.24 minutes. This means an average of 9 seconds that were added to the total build time which represents only 2.9\%. TODO... what else???

\begin{figure}
		\centering   
			\includegraphics[scale=0.26]{img/build-time-boxplots}
		\caption{Box plots comparing the MSBuild time of CMS solution with and without BugHunter analyzers. Each dataset contains 100 measurements.}
...
\begin{itemize}
\item[\textbf{Performance:}] Had they been experiencing any performance issues after BugHunter deployment? If so, what IDE set up were the developers using?

\item[\textbf{Correctness:}] Had they encountered any false positives/negatives while using the tool?

\item[\textbf{Code fixes:}] Had they already used any suggested code fix? Do they find it useful?
\end{itemize}

Out of approximately 19 developers currently working on the product, 10 participated in the survey. None of them complained about any performance degradation. The interesting fact to find out was, that 60\% do not use any third party tools for static code analysis, while the rest uses mostly ReSharper.

When asked about the false positive or false negative results from the tool, vast majority responded that they did not encounter any, while the others answered \textit{"Not sure"}.

Only 40\% of the respondents claimed to had already used a suggested code fix. This could be caused by the fact, that most of them had been working on the product for a long time and simply were not suggested any code fix. However, one half found the code fixes useful (the others responded "Not sure") and 7 out of 10 participants believed the code fixes will help the new developers to learn the internal CMS guidelines more quickly.
...
\chapter{Conclusion}
  ...
  - migration to Roslyn 2.0 (backward compatibility for customers, VS2017)
