...
The compilation process starts with \textit{lexical analysis} or \textit{scanning}. The scanner transforms stream of characters of the source program, as written by the programmer, into the series of meaningful sequences called \textit{lexemes}. Most programming languages allow for an arbitrary number of white spaces to be present in the source text to aid readability. However, white spaces, similarly as comments, are unimportant for the target code generation itself, and thus lexical analysis is responsible for discarding them completely.

In order to be able to recognize the correct lexeme to be sent to parser, lexical analyzer may need to read ahead. For example, in C--like languages if the scanner sees \texttt{<} character, it cannot decide whether it is a lexeme for "less then" operator or it is s part of "less then or equal to" lexeme. In order to do that, it needs to read ahead and see if the following character is \texttt{=} or not. Reading ahead is usually implemented with an input buffer which the lexical analyzer can read from and push back to. The use of buffer also boosts the performance as fetching block of characters is more efficient as fetching one at a time~\cite{dragon-book}.

The lexical analyzer typically uses regular expressions to identify the lexemes and for each lexeme, it outputs a \textit{token} (or \textit{token object}) of the form $$ \langle token \textnormal{-} name, attribute \textnormal{-} name \rangle $$
\noindent
...
Lexemes can be divided into logical groups such as identifiers, relational operators, arithmetical operators, constants or keywords as seen int the example above. Each group is defined by a regular expression. The starting nonterminals of these regular grammars are then used as terminal symbols of context free grammar (CFG) used in parsing phase.

Each identifier (\texttt{id}) has an attribute which points to the symbol table entry of the discovered identifier, where information about identifier name, type or position in the source text is stored. Similar holds for constants like "42" in the example. The assignment and addition symbols do not have attributes but different representation can be used, such as $ \langle bin \textnormal{-} op, 2 \rangle $, where \texttt{bin-op} would denote it is a binary operator and number two would be a pointer to symbol table with all symbols for binary operations with the second index suggesting that it represents addition.

  \section{Syntax Analysis}
The stream of token objects along with partially populated symbol table is an input for the subsequent compiler phase -- \textit{syntax analysis} or \textit{parsing}. The parser has to verify that the sequence of token names can by produced by the grammar for the source language and for a well-formed program, it shall output a \textit{syntax tree}. The syntax tree is an intermediate representation of source program in which each interior node represents an operation with the children of the node representing the arguments of that operation. 

The resulting syntax tree for the token stream generated in (2.2) is
\begin{center}
\begin{tikzpicture}
...
\begin{description}
  \item[Terminals] -- token names (first component of the token) as obtained from previous compilation step.
  ...
  \item[Start symbol] -- a special nonterminal which set of strings it denotes represents the language generated by the grammar.
...
An example of a production rule denoting the construction of a \texttt{while-cycle} would be
\begin{equation}
  stmt 
  \rightarrow 
  \textbf{while}\ 
...
\end{equation}
\noindent
where nonterminals \textit{stmt} and \textit{expr} stand for a statement and expression respectively. Symbols in bold represent terminals of the grammar -- open and close parenthesis and curly braces, plus \texttt{while} keyword.
