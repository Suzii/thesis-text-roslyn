...
It is parser's responsibility to report the~presence of potential syntactic error and recover from the~error in order to continue with syntactic analysis and be able to detect any subsequent errors. In~\cite{dragon-book}, two main strategies for the~error recovery are presented: \textit{panic-mode} and \textit{phrase-level} recovery.
...
\item Install\footnote{Installation of NuGet packages via~Visual Studio Package Manager UI is described in \url{https://docs.microsoft.com/en-us/nuget/tools/package-manager-ui\#finding-and-installing-a-package}. Note that the~\textit{package source} must set to~local feed.} one or both analyzer NuGets to the~desired projects.
\end{enumerate}
...
There are numerous ways to~disable an~analyzer for a~project, file, method, or a~single line of~code. 
