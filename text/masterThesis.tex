...
% TODO - mention Kentico is selling  the product for other developer - eg. they are selling api and the code, hosting and other stuff must be done by the customer
...
If the programmer uses the most common approach\footnote{Accessing the static property of the class that is handling the request.} of accessing the \texttt{Cookies} collection on the \texttt{Request} object, everything is fine and the BugHunter correctly reports the problem. However, it has no means of detecting other possible accesses to the property. If the \texttt{Request} object is passed to a function as an argument, or stored in a variable, the accesses of the cookies collection will not be detected by BugHunter. 
...
This category of analyzers makes sure that string comparison (\texttt{Equals()}, \texttt{Compare}, \texttt{IndexOf} and their variants) and manipulation methods (\texttt{ToLower(), \texttt{ToUpper()}}) are always used with the overload specifying \texttt{StringComparison} or \texttt{CultureInfo} parameter explicitly. This is vital for an application like CMS that is used with culture specific data. For more information on what issues can be caused when not following this rule, like the \textit{"Turkish-I problem"}, see \cite{best-practices-for-using-strings-in-dot-net} (https://msdn.microsoft.com/en-us/library/dd465121(v=vs.110).aspx). 
