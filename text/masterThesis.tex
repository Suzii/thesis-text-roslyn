...
This holds for any custom analyzers written, but even more so if they are to be used with large solutions like CMS solution. Developers at Kentico complain about the responsiveness of the Visual Studio with opened CMS Soultion. Some of them even admitted they had uninstalled the ReSharper extension, since it slow down the IDE to an unbearable extent and claimed, that the plugin often caused IDE crashes. 
...
Before discussing the performance of BugHunter analzyers, it is important to understand the structure of the CMS solution. This is helpful when considering which parts of the analyzers code are crucial for the overall performance an which can be neglected.

In order to collect relevant information, \texttt{SolutionStatistics} project was created (the source code can be found in the IS attachment). The project loads the CMS solution and collects information on number of projects, documents, syntax nodes. The summary of the results is presented in Table~\ref{tab:solution-statistics}. Note, that just the data relevant for implemented analyzers are displayed. Therefore, only projects where NuGet with BugHunter analzyers is installed\footnote{Test and 3rd party projects were excluded from statistics, see Appendix~\ref{appendix:deployment}.} and relevant kinds of syntax nodes are considered.
...
In the early stages of the development, the initiative was to compute benchmarks for different analyzer versions using BenchmarkDotNet framework\footnote{https://github.com/dotnet/BenchmarkDotNet}. However, the overhead of creating the compilation together with the cost of the analyzers' execution were too big for the results to be useful. 
...
Before considering the overall impact of BugHunter anlayzers on the compilation of the CMS solution, it is important to tune the performance of separate analyzers. The only way that Microsoft provides for measuring the analyzers' execution times is \texttt{/ReportAnalyzer} switch in CSC (CSharp compiler) process.

As stated in~\cite{report-analyzer}, if the compiler is invoked with \texttt{/ReportAanlyzer} command line switch, its output will contain total wall clock time spent in executing the analyzers along with relative times of separate analyzers. Since the analyzers can run concurrently, the total time should only be treated as the upper bound. The relative times per analyzer are useful for comparing their performance and identifying any outliers.
...
\subsubsection{Parsing and Aggregating Per-project Results}
The resulting MSBuild output has almost 500MB and besides normal compiler output also contain information on analyzers' execution times for every project that the analyzers' NuGet is installed to. For CMS solution this means 125 projects. For the purpose of this thesis, however, only performance over whole solution is relevant. 

In order to parse the results from the large output, a console application \texttt{ReportAnalyzerTimes.Parser} was developed. It extracts all information on analyzer execution times from the log (based on the format described in~\cite{report-analyzer}. After that, it sums up the absolute execution times \footnote{Smallest value, that can be reported, for the execution time is \texttt{<0.001s} and it is treated as 0.001s by the parsing application.} for same analyzers across the solution (sums per-project results) and outputs them into a file.

\subsubsection{Aggregating Results from Multiple Runs}
Since a single compiler run cannot be treated as ...TODO..., a PowerShell script (Report-Analyzer.ps1), executes the MSBuild of the CMS solution multiple times, was written. After every run, it run ReportAnalyzerTimes.Parser application and results from all runs are saved in multiple text files. 
...
Why no statefull analyzers that have compilation end actions -- to large solutions it can take more than one minute for the diagnostic to show up
