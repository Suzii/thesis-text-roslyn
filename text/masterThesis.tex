...
The procedure described in the previous section was used to measure and compare the performance of implemented analyzers. The measurements were performed on the CMS solution, (TODO delete the following???) as well as on an artificially generated project with uniformly distributed diagnostics for each analyzer (source codes attached in IS).
...
TODO delete this subsection completely and only present the final optimization???
...
To prevent multiple inspections of same syntax tree for all its identifier name nodes, the information whether the tree contains \texttt{System.IO} using is cached in a concurrent dictionary. This optimization technique managed to shrink the analyzer's execution time on CMS solution to an average of 2.40 seconds, whereas the non-cached version took 10.94 seconds on average. 

The execution times of final analyzer (with non-cached version as well), V1 analyzer and artificial abnalyzers serving as baselines are depicted in a box plot diagram in Figure~\ref{fig:system-io-versions-boxplots}.
...
\section{Influence of BugHunter analyzers on CMS Solution build time}
