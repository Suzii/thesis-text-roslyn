...
In order to understand the chapters describing the implementation part of the thesis, it is vital to know how the Diagnostic APIs work and how they can be leveraged to write a custom analyzer. This section should provide a quick introduction to analyzers and code fixes in Roslyn.

\subsection{Diagnostic Analyzer}
An analyzer is an instance of a type deriving from \texttt{Microsoft.CodeAnalysis.Diagnostics.DiagnosticAnalyzer} which must be annotated with \texttt{DiagnosticAnalyzer} attribute specifying the targeted programming language (C\# or Visual Basic)\cite{roslyn-succinctly}. In the thesis, these instances are referred to as \textit{"end-analyzers"}, in order to distinguish them from abstract analyzers or helper classes used for analysis. End-analyzer can contain one or more custom \textit{rules} for detecting domain specific errors or code issues. Each such rule is defined by \texttt{DiagnosticDescriptor}. Once the analyzer finds an issue, the diagnostic descriptor is used to create a \textit{diagnostic} which also includes data collected by the compiler, such as location.

\subsubsection{Diagnostic Descriptor}
For any Roslyn end-analyzer it is mandatory to override an immutable property \texttt{SupportedDiagnostics}, that returns an immutable array of diagnostic descriptors. These contain:

\begin{compactitem}
  \item \textbf{\texttt{DiagnosticId}} -- value uniquely identifying a single rule (for example "BH1014").
  \item \textbf{\texttt{Title}}, \textbf{\texttt{MessageFormat}}, \textbf{\texttt{Description}} -- localizable strings that will be displayed in Error List or diagnostic's tooltip on hover. The message format can be also passed arguments upon diagnostic creation, to give more details about the concrete issue detected.
  ...
  \item \texttt{\textbf{DefaultSeverity}} -- one of \texttt{Error}, \texttt{Warning}, \texttt{Info}, \texttt{Hidden}, \texttt{None}. Note, that only warning prevents the project from compiling successfully.
...
\begin{compactitem}
  ...
  \item \texttt{\textbf{RegisterSemanticModelAction}} -- the action will be invoked once per document, after the semantic analysis of the document is finished.
  
  \item \texttt{\textbf{RegisterCompilationAction}} -- the action will be invoked once per compilation.
  
  \item \texttt{\textbf{RegisterCompilationStartAction}} -- the action will be invoked when the compilation starts, before any other actions. The context can be then used to register other actions within the compilation as well as the corresponding \texttt{\textbf{RegisterCompilationEndAction}}.
  
\item \texttt{\textbf{RegisterCodeBlockAction}} -- the action will be invoked once per code block.
...
Start actions are usually used for statefull analyzers... note that more actions can be registered . Most common is syntax node on different kinds.

Important not to throw an exception...

... To give an overview how a typical simple diagnostic analyzer looks like,... tadaa screenshot...veeeeery simplyfied api replacement analyzer...

\subsection{Code Fix Provider}
Give screenshot of api replacement analyzer code fix preview 
- what it can do

- how

- DiagnosticAnalyzer
https://github.com/dotnet/roslyn/wiki/Getting-Started-Writing-a-Custom-Analyzer-\&-Code-Fix  
-- registering actions
...
- Gotchas
-- not storing state that wont be garbage-collected
-- no exceptions otherwise VS will disable the analyzer / codefix
-- performance considerations
-- code fix vs code refactoring
...
% my opininon on MS docummentation...
