...
Start with Clearly stating the goal of the measurements. Only then define the metric (To define a goal have a look at slides 66+) ... where the measurements were conducted (developer machine i5, 16GB of ram)
Goal-Question-Metric

"I would maybe rewrite this in more accurate form, in the sense that when measuring performance in an empirical way one needs to consider several runs and define a-priori some statistical measures (e.g. discarding all values +/- stand deviation from the mean). Results will always be an approximation, but given large number of runs they should approximate quite well  "

"It might be useful, I would see this section that starts with the problems of measuring performance in a rigorous/objective way, lists the common problems, but then discusses the approach taken. In the approach taken, I would also mention what you decided to consider as number of runs and ways to reduce the impact of outliers. Some Activity or sequence diagram could be useful"
--------- TODO - rewrite??? -------
...
\begin{figure}
		...
		\caption{Box plots comparing of CMS solution build time without any analyzers compared to build time with BugHunter analyzers. 100 runs.}
